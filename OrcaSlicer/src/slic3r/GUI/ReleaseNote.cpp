    ...
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe), wxID_ANY, _L("Network plug-in update"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    operation_tips = new ::Label(this, Label::Body_12, _L("Click OK to update the Network plug-in when Sovol Slicer launches next time."), LB_AUTO_WRAP);
...
UpdateVersionDialog::UpdateVersionDialog(wxWindow *parent)
    : DPIDialog(parent, wxID_ANY, _L("New version of Sovol Slicer"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
    std::string icon_path = (boost::format("%1%/images/SovolSlicerTitle.ico") % resources_dir()).str();
    SetIcon(wxIcon(encode_path(icon_path.c_str()), wxBITMAP_TYPE_ICO));
    ...
    auto sm    = create_scaled_bitmap("SovolSlicer", nullptr, 70);
    ...
    :DPIFrame(parent, id, title, pos, size, style)
{
    ...
    auto sizer_button = new wxBoxSizer(wxHORIZONTAL);
    btn_bg_green = StateColor(std::pair<wxColour, int>(wxColour(0, 142, 172), StateColor::Pressed), std::pair<wxColour, int>(wxColour(38, 171, 203), StateColor::Hovered),
        std::pair<wxColour, int>(AMS_CONTROL_BRAND_COLOUR, StateColor::Normal));
...
{
    m_button_ok->Enable();
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(38, 171, 203), StateColor::Pressed), std::pair<wxColour, int>(wxColour(38, 171, 203), StateColor::Hovered),
        std::pair<wxColour, int>(AMS_CONTROL_BRAND_COLOUR, StateColor::Normal));
    m_button_ok->SetBackgroundColor(btn_bg_green);
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    comfirm_before_enter_text = _L("Step 1. Please confirm Sovol Slicer and your printer are in the same LAN.");
    ...
    m_test_right_msg->SetForegroundColour(wxColour(38, 171, 203));
